...
`onyx-data-lineage-pipe` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies lineage behavior through negative fixtures, with human-readable error snapshots and local-only command execution.
...
The score is deterministic. The current threshold is 177. Risk has a penalty of 4, latency has a penalty of 2, and weight contributes a bonus of 5.
